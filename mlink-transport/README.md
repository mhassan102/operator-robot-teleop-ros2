...
> Default doc for `mlink-transport/` (and the `mlink-support` branch).
> Product roadmap: [`../IMPLEMENTATION.md`](../IMPLEMENTATION.md)
> (Features F3 / F8 / F9). How to run tests and lab demos:
...
**Done means:** `pytest` (or equivalent) all green; `docs/usage.md`
how to run tests; header + config schema documented in this file
(keep them matching).

**Code the next session should read:** this file §8 Stage 2;
`mlink-transport/docs/usage.md`; `mlink-transport/proto/` (especially
...
- **Verify:** `cd mlink-transport && python3 -m pytest` — 34 passed
  ...
  commands in `mlink-transport/docs/usage.md`.
...
- `docs/usage.md`: exact commands to run op, edge, ping, and the kill-path demo.
...
`mlink-transport/docs/usage.md`; `daemon.py`; `ping.py`; `proto/sockets.py`
...
- `docs/usage.md`: assign Ethernet IPs without changing default route; copy
...
`docs/stage3_overview.md`; `mlink-transport/docs/usage.md` Stage 3 section.
...
- `docs/usage.md`: how to run unit tests and the loopback demo
